...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _buildStatsRow() {
    return Row(
      children: [
        _statTile(label: 'WEIGHT', value: _displayWeight.toStringAsFixed(1), unit: _weightLabel, icon: Icons.monitor_weight_outlined),
        const SizedBox(width: 10),
        _statTile(label: 'HEIGHT', value: _displayHeight.toStringAsFixed(_heightInCm ? 0 : 1), unit: _heightLabel, icon: Icons.height),
        const SizedBox(width: 10),
        _statTile(label: 'GOAL WT', value: _displayGoal.toStringAsFixed(1), unit: _weightLabel, icon: Icons.flag_outlined, highlight: true),
      ],
    );
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              label,
              style: TextStyle(
                color: Colors.white.withOpacity(0.35), fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 0.8,
              ),
            ),
          ],
        ),
    ...
    return Column(
      ...
      children: [
        Text(
          title,
          style: TextStyle(
            color: Colors.white.withOpacity(0.45), fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1.5,
          ),
        ),
        const SizedBox(height: 10),
        content,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(label, style: TextStyle(color: Colors.white.withOpacity(0.45), fontSize: 12)),
          ),
          child,
        ],
      ),
      ...
      child: Column(
        children: [
          _metricRow(
            ...
            unit: _weightLabel,
            onMinus: _isEditing ? () => setState(() => _weight = (_weight - 0.5).clamp(30, 300)) : null,
            onPlus: _isEditing ? () => setState(() => _weight = (_weight + 0.5).clamp(30, 300)) : null,
            unitToggle: _isEditing
                ? _buildUnitToggle(['KG', 'LBS'], _weightInKg, (kg) {
                    setState(() {
                      if (_weightInKg != kg) {
          ...
          _metricRow(
            ...
            onMinus: _isEditing
                ? () => setState(() => _height = (_height - (_heightInCm ? 1 : 0.01)).clamp(100, 250))
                : null,
            onPlus: _isEditing
                ? () => setState(() => _height = (_height + (_heightInCm ? 1 : 0.01)).clamp(100, 250))
                : null,
            unitToggle: _isEditing
                ? _buildUnitToggle(['CM', 'FT'], _heightInCm, (cm) {
                    setState(() {
                      if (_heightInCm != cm) {
          ...
          _metricRow(
            ...
            unit: _weightLabel,
            isLast: true,
            onMinus: _isEditing ? () => setState(() => _weightGoal = (_weightGoal - 0.5).clamp(30, 300)) : null,
            onPlus: _isEditing ? () => setState(() => _weightGoal = (_weightGoal + 0.5).clamp(30, 300)) : null,
          ),
        ],
      ),
    );
  ...
  Widget _metricRow({
    ...
    final isEditing = _isEditing && (onMinus != null || onPlus != null);
    return Padding(
